...
impl Mechanism {
    ...
    pub fn four_bar(p0: (f64, f64), l0: f64, l1: f64, l2: f64, l3: f64, l4: f64, g: f64) -> Self {
        let mut s = Self {
            joints: Vec::with_capacity(5),
            formulas: vec![],
        };
        s.joints.push([p0.0, p0.1]);
        s.joints.push([p0.0 + l0, 0.]);
        for _ in 2..5 {
            s.joints.push([0., 0.]);
        }
        s.formulas.push(Formula::PLAP(0, l1, 0., 1, 2));
    ...
    pub fn four_bar_angle(&mut self, angle: f64) -> Result<()> {
        if let Formula::PLAP(_, _, ref mut a, _, _) = self.formulas[0] {
            *a = angle;
            Ok(())
        } else {
            Err(Error::new(ErrorKind::AddrNotAvailable, "invalid four bar"))
        }
    }
