...
pub(crate) struct SynConfig {
    ...
    pub(crate) res: usize,
    /// Specify the origin of the result to coordinate origin
    #[cfg_attr(not(target_arch = "wasm32"), clap(long))]
    pub(crate) no_trans: bool,
...
impl<'a> Solver<'a> {
    ...
    {
        let SynConfig { seed, gen, pop, mode, res, no_trans } = cfg;
        macro_rules! impl_solve {
            ($target:ident, $cb:ident, $syn:ident) => {{
                let mut s = syn::$syn::from_curve(&$target, mode).res(res);
                if no_trans {
                    s = s.origin(Default::default());
                }
                let mut s = method
                    .build_solver(s)
                    .seed(seed)
